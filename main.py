...
def draw(client: Client, message: Message):
    ...
    if message.from_user.id == client.get_me().id:
        if message.text.split(' ',1)[1] == '+alldraw':
            alldraw = 1
            message.edit(ln(31)[7])
        elif message.text.split(' ',1)[1] == '-alldraw':
            alldraw = 0
            message.edit(ln(31)[8])
        else:
            try:
                prompt = message.text.split(' ',1)[1]
                message.edit(ln(31)[5])
                response = openai.Image.create(prompt=prompt, n=1, size='1024x1024')
                image_url = response["data"][0]["url"]
                file_path = './bin/hash/cache/photo_draw.jpg'
                download_image(image_url, file_path)
                message.delete()
                with open(file_path, 'rb') as photo:
                    client.send_photo(message.chat.id, photo, 'Made with [Telehash](https://t.me/telehashdev)\nPrompt: {}'.format(prompt))
            except Exception as e: print(e)
    else:
        if alldraw == 1:
            try:
                prompt = message.text.split(' ',1)[1]
                msgpt = client.send_message(message.chat.id, ln(31)[5], reply_to_message_id=message.id)
                response = openai.Image.create(prompt=prompt, n=1, size='1024x1024')
                image_url = response["data"][0]["url"]
                file_path = './bin/hash/cache/photo_draw.jpg'
                download_image(image_url, file_path)
            ...
            client.send_message(message.chat.id, ln(31)[6])
...
# IN TEST
'''
# Команда для импорта модуля
@client.on_message(filters.command("import", prefixes="!"))
def module_import(client: Client, message: Message):
    # Получение имени модуля из аргументов команды
    module_name = message.command[1] if len(message.command) > 1 else ""
    try:
        # Импорт модуля
        import_module(module_name)
        message.edit(f"Модуль {module_name} успешно импортирован!")
    except ImportError:
        message.edit(f"Модуль {module_name} не найден!")
'''
# MODULES
# IN TEST
